...
from ..tap_payments import (
    TapPaymentError,
    TapPaymentValidationError,
    create_charge as tap_create_charge,
    customer_payload as tap_customer_payload,
    retrieve_charge as tap_retrieve_charge,
    tap_is_configured,
    verify_webhook_hash as tap_verify_webhook_hash,
)
...
def _tap_absolute_url(route_name: str) -> str:
    site_url = str(getattr(settings, "SITE_URL", "") or "").strip().rstrip("/")
    if not site_url:
        raise TapPaymentError("رابط المنصة العام غير مضبوط.")
    return f"{site_url}{reverse(route_name)}"


def _tap_selected_items(request, membership, subscription):
    school = membership.school
    pricing = _archive_pricing()
    include_sub = (request.POST.get("include_subscription") or "") == "1"
    include_addon = (request.POST.get("include_archive_addon") or "") == "1"
    include_storage = (request.POST.get("include_archive_storage") or "") == "1"
    if not (include_sub or include_addon or include_storage):
        raise TapPaymentValidationError("اختر خدمة واحدة على الأقل للدفع.")

    selected_purposes = []
    if include_sub:
        selected_purposes.append(Payment.Purpose.SUBSCRIPTION)
    if include_addon:
        selected_purposes.append(Payment.Purpose.ARCHIVE_ADDON)
    if include_storage:
        selected_purposes.append(Payment.Purpose.ARCHIVE_STORAGE)
    if Payment.objects.filter(
        school=school,
        purpose__in=selected_purposes,
        status=Payment.Status.PENDING,
    ).exists():
        raise TapPaymentValidationError(
            "يوجد طلب سابق قيد المعالجة لإحدى الخدمات المحددة. انتظر نتيجته قبل إنشاء طلب جديد."
        )
    ...
    if include_sub:
        ...
        if (
            ...
            requested_plan = subscription.plan
        if requested_plan is None:
            raise TapPaymentValidationError("اختر باقة مدفوعة متاحة لتجديد اشتراك المدرسة.")
        items.append(
            {
                "purpose": Payment.Purpose.SUBSCRIPTION,
                "requested_plan": requested_plan,
                "amount": Decimal(requested_plan.price),
        ...
        items.append(
            {
                "purpose": Payment.Purpose.ARCHIVE_ADDON,
                "requested_plan": None,
                "amount": Decimal(pricing["addon_price"]),
                "archive_storage_gb": 0,
                "label": "إضافة الأرشفة السنوية",
            }
        )
    ...
    if include_storage:
        ...
        if not (addon and addon.is_active) and not include_addon:
            raise TapPaymentValidationError(
                "زيادة مساحة الأرشيف تتطلب تفعيل إضافة الأرشفة ضمن الطلب نفسه أو مسبقًا."
            )
        option_id = request.POST.get("archive_storage_option_id")
        option = ArchiveStorageOption.objects.filter(pk=option_id, is_active=True).first()
        if option is None:
            raise TapPaymentValidationError("اختر خيار زيادة مساحة صالحًا.")
        items.append(
            {
                "purpose": Payment.Purpose.ARCHIVE_STORAGE,
                "requested_plan": None,
    ...
    if not items:
        raise TapPaymentValidationError("لا توجد خدمات صالحة للدفع.")
    return items


def _tap_charge_payments(charge):
    charge_id = str(charge.get("id") or "").strip()
    reference = charge.get("reference") if isinstance(charge.get("reference"), dict) else {}
    order_ref = str(reference.get("order") or "").strip()
    batch_ref = order_ref[4:] if order_ref.startswith("ord_") else ""
    if not batch_ref or len(batch_ref) > 32:
        raise TapPaymentValidationError("مرجع طلب Tap لا يطابق أي طلب في المنصة.")
    ...
    if not payments:
        raise TapPaymentValidationError("لم يُعثر على طلب الدفع المرتبط بعملية Tap.")
    existing_ids = {p.transaction_id for p in payments if p.transaction_id}
    if existing_ids and existing_ids != {charge_id}:
        raise TapPaymentValidationError("رقم عملية Tap لا يطابق الطلب المحلي.")
    if any(p.school_id != payments[0].school_id for p in payments):
        raise TapPaymentValidationError("بيانات طلب الدفع المحلي غير متسقة.")
    return payments


def _tap_validate_charge(charge, payments):
    charge_id = str(charge.get("id") or "").strip()
    if not charge_id.startswith("chg_"):
        raise TapPaymentValidationError("رد Tap لا يحتوي على رقم عملية صالح.")

    expected_currency = str(getattr(settings, "TAP_CURRENCY", "SAR") or "SAR").upper()
    actual_currency = str(charge.get("currency") or "").upper()
    if actual_currency != expected_currency:
        raise TapPaymentValidationError("عملة عملية Tap لا تطابق عملة الطلب.")

    expected_amount = sum((Decimal(p.amount) for p in payments), Decimal("0")).quantize(Decimal("0.01"))
    try:
        actual_amount = Decimal(str(charge.get("amount"))).quantize(Decimal("0.01"))
    except Exception as exc:
        raise TapPaymentValidationError("مبلغ عملية Tap غير صالح.") from exc
    if actual_amount != expected_amount:
        raise TapPaymentValidationError("مبلغ عملية Tap لا يطابق إجمالي الطلب.")

    merchant = charge.get("merchant") if isinstance(charge.get("merchant"), dict) else {}
    actual_merchant = str(merchant.get("id") or charge.get("merchant_id") or "").strip()
    expected_merchant = str(getattr(settings, "TAP_MERCHANT_ID", "") or "").strip()
    if actual_merchant and expected_merchant and actual_merchant != expected_merchant:
        raise TapPaymentValidationError("حساب التاجر في عملية Tap لا يطابق حساب المنصة.")

    secret_key = str(getattr(settings, "TAP_SECRET_KEY", "") or "")
    live_mode = charge.get("live_mode")
    if secret_key.startswith("sk_live_") and live_mode is False:
        raise TapPaymentValidationError("تم استلام عملية تجريبية باستخدام إعدادات الإنتاج.")
    if secret_key.startswith("sk_test_") and live_mode is True:
        raise TapPaymentValidationError("تم استلام عملية إنتاج باستخدام إعدادات الاختبار.")


def _tap_process_charge(charge):
    payments = _tap_charge_payments(charge)
    _tap_validate_charge(charge, payments)
    charge_id = str(charge.get("id") or "").strip()
    gateway_status = str(charge.get("status") or "").strip().upper()
    response_data = charge.get("response") if isinstance(charge.get("response"), dict) else {}
    response_code = str(response_data.get("code") or "")[:32]
    response_message = str(response_data.get("message") or "")[:255]
    terminal_statuses = {
        "ABANDONED": Payment.Status.CANCELLED,
        "CANCELLED": Payment.Status.CANCELLED,
        "TIMEDOUT": Payment.Status.CANCELLED,
        "FAILED": Payment.Status.REJECTED,
        "DECLINED": Payment.Status.REJECTED,
        "RESTRICTED": Payment.Status.REJECTED,
        "VOID": Payment.Status.REJECTED,
        "UNKNOWN": Payment.Status.REJECTED,
    }

    with transaction.atomic():
        locked = list(
            Payment.objects.select_for_update()
            .select_related("school", "subscription", "requested_plan")
            .filter(pk__in=[p.pk for p in payments])
            .order_by("id")
        )
        now = timezone.now()
        for payment in locked:
            ...
            payment.gateway_message = response_message
            update_fields = [
                "transaction_id",
                "gateway_status",
            ...
            if gateway_status == "CAPTURED":
                ...
                payment.gateway_completed_at = now
                update_fields.extend(["status", "payment_date", "gateway_completed_at"])
            elif gateway_status in terminal_statuses and payment.status != Payment.Status.APPROVED:
                payment.status = terminal_statuses[gateway_status]
                payment.gateway_completed_at = now
                update_fields.extend(["status", "gateway_completed_at"])
        ...
        if gateway_status == "CAPTURED":
            ...
            ordered = sorted(
                locked,
                key=lambda p: (_PURPOSE_APPLY_ORDER.get(p.purpose, 99), p.pk),
            )
...
@ratelimit(key="user", rate="5/m", method="POST", block=True)
@require_http_methods(["POST"])
def tap_payment_create(request):
    if not tap_is_configured():
        messages.error(request, "بوابة Tap غير مهيأة بعد. استخدم التحويل البنكي أو تواصل مع إدارة المنصة.")
        return redirect("reports:my_subscription")
    ...
    membership = memberships.filter(school=active_school).first() if active_school else None
    membership = membership or memberships.first()
    if not membership:
        messages.error(request, "هذه الصفحة مخصصة لمدير المدرسة فقط.")
        return redirect("reports:home")
    ...
    try:
        items = _tap_selected_items(request, membership, subscription)
    except TapPaymentError as exc:
        messages.error(request, str(exc))
        return redirect("reports:my_subscription")

    batch_ref = uuid.uuid4().hex
    total = sum((item["amount"] for item in items), Decimal("0")).quantize(Decimal("0.01"))
    labels = "، ".join(item["label"] for item in items)
    user_notes = (request.POST.get("notes") or "").strip()
    base_note = f"[Tap {batch_ref[:8]}] {labels} — الإجمالي {total} ريال."
    if user_notes:
        base_note = f"{base_note}\nملاحظة المدير: {user_notes[:500]}"
    ...
    with transaction.atomic():
        payments = [
            Payment.objects.create(
                ...
                amount=item["amount"],
                archive_storage_gb=item["archive_storage_gb"],
                batch_ref=batch_ref,
                payment_method=Payment.Method.TAP,
                notes=base_note,
                created_by=request.user,
    ...
    payload = {
        "amount": float(total),
        "currency": str(getattr(settings, "TAP_CURRENCY", "SAR") or "SAR").upper(),
        "customer_initiated": True,
        "threeDSecure": True,
        "save_card": False,
        "description": f"خدمات منصة توثيق - {membership.school.name}"[:255],
        "metadata": {
            "batch_ref": batch_ref,
        ...
        "customer": tap_customer_payload(request.user),
        "merchant": {"id": str(getattr(settings, "TAP_MERCHANT_ID", "") or "")},
        "source": {"id": str(getattr(settings, "TAP_SOURCE_ID", "src_all") or "src_all")},
        "post": {"url": _tap_absolute_url("reports:tap_payment_webhook")},
        "redirect": {"url": _tap_absolute_url("reports:tap_payment_return")},
    }

    try:
        charge = tap_create_charge(payload)
        charge_id = str(charge.get("id") or "").strip()
        if not charge_id.startswith("chg_"):
            raise TapPaymentError("لم تُرجع بوابة Tap رقم عملية صالحًا.")
        gateway_status = str(charge.get("status") or "").strip().upper()
    ...
    if gateway_status == "CAPTURED":
        try:
            _tap_process_charge(charge)
        except TapPaymentError:
            logger.exception("Tap direct charge validation failed for %s", charge_id)
            messages.error(request, "تمت العملية لدى Tap، لكن تعذّر مطابقتها تلقائيًا. تواصل مع الدعم برقم العملية.")
        else:
            messages.success(request, "تم الدفع وتفعيل الخدمات بنجاح.")
        return redirect("reports:my_subscription")

    transaction_data = charge.get("transaction") if isinstance(charge.get("transaction"), dict) else {}
    payment_url = str(transaction_data.get("url") or "").strip()
    parsed_url = urlparse(payment_url)
    if parsed_url.scheme != "https" or not parsed_url.netloc:
        Payment.objects.filter(pk__in=[p.pk for p in payments]).update(
            status=Payment.Status.CANCELLED,
            gateway_status="INVALID_REDIRECT",
            gateway_message="Tap did not return a secure payment URL.",
            gateway_completed_at=timezone.now(),
        )
        messages.error(request, "لم تُرجع بوابة Tap رابط دفع آمنًا. حاول مرة أخرى.")
        return redirect("reports:my_subscription")
    return redirect(payment_url)


@require_http_methods(["GET"])
def tap_payment_return(request):
    tap_id = str(request.GET.get("tap_id") or "").strip()
    if not tap_id:
        messages.error(request, "لم يصل رقم عملية Tap.")
        return redirect("reports:my_subscription")
    try:
        charge = tap_retrieve_charge(tap_id)
        status = _tap_process_charge(charge)
    except TapPaymentError as exc:
        logger.warning("Tap return could not be processed for %s: %s", tap_id[:48], exc)
        messages.error(request, "تعذّر التحقق من نتيجة الدفع. إذا خُصم المبلغ فتواصل مع الدعم برقم العملية.")
    else:
        if status == "CAPTURED":
            messages.success(request, "تم الدفع بنجاح وتفعيل الخدمات المحددة تلقائيًا.")
        elif status in {"INITIATED", "PENDING"}:
            messages.info(request, "عملية الدفع لا تزال قيد المعالجة لدى Tap.")
        else:
            messages.error(request, "لم تكتمل عملية الدفع. يمكنك المحاولة مرة أخرى.")
    return redirect("reports:my_subscription")


@csrf_exempt
@require_http_methods(["POST"])
def tap_payment_webhook(request):
    if not tap_is_configured():
        return JsonResponse({"detail": "Tap is not configured."}, status=503)
    ...
    try:
        verified = tap_verify_webhook_hash(payload, posted_hash)
    except TapPaymentError:
        verified = False
    if not verified:
        return JsonResponse({"detail": "Invalid signature."}, status=400)
    try:
        status = _tap_process_charge(payload)
    except TapPaymentError as exc:
        logger.warning("Tap webhook rejected for %s: %s", str(payload.get("id") or "")[:48], exc)
        return JsonResponse({"detail": "Charge validation failed."}, status=400)
