...
def _one_request(url: str, timeout: float) -> tuple[str, float]:
    started = time.perf_counter()
    request = urllib.request.Request(url, headers={"User-Agent": "tawtheeq-loadtest/1.0"})
    try:
...
def run(url: str, concurrency: int, total: int, timeout: float) -> int:
        ...
        while True:
            with lock:
                if counter["issued"] >= total:
                    return
                counter["issued"] += 1
            outcome, elapsed = _one_request(url, timeout)
            with lock:
                outcomes[outcome] += 1
